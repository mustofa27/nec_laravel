...
jQuery(function(){
	...
	legend_width = jQuery('#legend').outerWidth();
	data_panel_height = jQuery('#data_panel').outerHeight();
	
	jQuery('#themes_panel').css('right', -themes_panel_width);
	jQuery('#themes_panel').animate({right: -themes_panel_width}, 400);

	jQuery('#legend').css('left', -legend_width);
	jQuery('#legend').animate({left: -legend_width}, 400);
	
	jQuery('#toggle_button').click(function() {

		var themes_panel = jQuery(this).parent();
		themes_panel.animate({
		  	right: parseInt(themes_panel.css('right'),10) == 0 ? -themes_panel_width : 0
		}, 400);

		var data_panel = jQuery('#data_panel');

		if(parseInt(themes_panel.css('right'),10) == 0) {
			data_panel.animate({ 
				bottom:-data_panel_height 
			}, 400);
			jQuery('#toggle_table').html("SHOW TABLE");
		}

		return false;
	});

	jQuery('#close_table_button').click(function() {
		var data_panel = jQuery('#data_panel');
		data_panel.animate({
		  bottom: -data_panel_height
		}, 400);
	...
	jQuery('#toggle_table').click(function() {
		var data_panel = jQuery('#data_panel');
		data_panel.animate({
		  bottom: parseInt(data_panel.css('bottom'),10) == 20 ? -data_panel_height : 20
		}, 400);

		if(parseInt(data_panel.css('bottom'),10) == 20) {
			jQuery(this).html("SHOW TABLE");
		} else {
			jQuery(this).html("HIDE TABLE");
		}
		return false;
	});
